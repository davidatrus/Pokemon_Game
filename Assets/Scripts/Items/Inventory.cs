...
public class Inventory : MonoBehaviour,ISavable
{
    ...
    public void AddItem(ItemBase item, int count = 1)
    {
       ...
       var itemSlot=currentSlots.FirstOrDefault(slot => slot.Item == item); //code to get item slot
        if (itemSlot != null)
        {
    ...
    public bool HasItem(ItemBase item)
    {
       ...
       return currentSlots.Exists(slot => slot.Item == item);//returing wehter or not we have the item 
    }
...
public class ItemSlot
{
    ...
    {
        item = ItemDB.GetObectByName(saveData.name);

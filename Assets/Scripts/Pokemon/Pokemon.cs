...
public class Pokemon
{
    ...
    {
        //constructor to restor data
        //restoring its name from pokemon database, hp, level and exp. conditional logic for if has status condition or not
        _base = PokemonDB.GetObectByName(saveData.name);
        HP = saveData.hp;
    ...
    public Evolution CheckForEvolution(ItemBase item)//function overloading. Same name just if evoling with item require an item if not dont require item
    {
    ...
    public DamageDetails TakeDamage(Move move ,Pokemon attacker)
    {
       ...
       float attack = (move.Base.Category==MoveCategory.Special) ? attacker.SpAttack : attacker.Attack;
        ...
        float modifers = Random.Range(0.85f, 1f) * type * critical;
    ...
    {
        bool canPerformMove = true; //before move can always perform move, goes into if statement checking if status changes its ability to perform move.
        ...
        return canPerformMove; //if it doesnt have any status changes, volatile or regular then perform the move.
    }
...
public class PokemonSaveData
{
    ...
    //using public because its only use to save data, so dw about making private
}
